...
function validAmount(value: unknown): string | null {
  ...
  if (whole.length > 10 || Number(normalized) <= 0) {
    return null;
  }
...
function validate(body: Record<string, unknown>) {
  ...
  if (
    !PAYMENT_METHODS.includes(
      body.payment_method as (typeof PAYMENT_METHODS)[number]
    )
  ) {
    return {
  ...
  if (!isValidTime(body.expense_time)) {
    return {
      error: "Enter a valid expense time.",
    };
  }
  ...
  return {
    value: {
      ...
      expense_time: body.expense_time as string,
      description:
...
export async function GET(request: Request) {
  try {
    ...
    if (date) {
      ...
      filters.push(
        gte(expenses.expense_date, bounds.start),
        lte(expenses.expense_date, bounds.end)
      );
    }
    ...
    if (payment) {
      ...
      filters.push(
        eq(expenses.payment_method, payment)
      );
    }
